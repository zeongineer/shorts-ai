...
def seconds_to_df_timecode(seconds: float) -> str:
    """
    초 단위 시간을 NTSC Drop Frame Timecode (29.97fps / HH:MM:SS;FF) 형식으로 변환한다.
    """
...
# ==============================================================================
# 4. Whisper STT (객체/Dict 안정적 파싱)
# ==============================================================================

def extract_segment_data(segment: Any) -> Dict[str, Any]:
    """
    Groq Whisper API의 반환 형태가 객체(Object)이거나 Dict인 경우
    모두 안전하게 값을 추출할 수 있도록 래핑하는 함수.
    """
...
def run_whisper_stt(client: Groq, audio_path: str) -> List[Dict[str, Any]]:
    ...
    parsed_segments = [extract_segment_data(seg) for seg in raw_segments]
    return parsed_segments


# ==============================================================================
# 5. 하이라이트 데이터 Python 단 검증 및 보정
# ==============================================================================
...
# ==============================================================================
# 6. Gemini 하이라이트 추출 (2026 최신 모델 최적화)
# ==============================================================================
...
def run_gemini_highlight_extraction(
    ...
    # 2026년 기준 사용 가능한 Gemini 모델 순서 설정
    preferred_models = [
        "gemini-3.7-flash",
        "gemini-3.6-flash",
        "gemini-2.5-flash",
...
def main():
    ...
    if uploaded_file is None:
        st.info("📌 파일 올려주시면 하이라이트 분석 및 EDL 생성을 시작할 수 있습니다.")
        return

    file_size_mb = uploaded_file.size / (1024 * 1024)
    st.success(f"📁 파일 선택 완료: **{uploaded_file.name}** ({file_size_mb:.2f} MB)")

    if uploaded_file.size > (1024 * 1024 * 1024):  # 1GB
        ...
        for index, highlight in enumerate(highlights, 1):
            ...
            st.markdown(
                ...
                <div class="highlight-card">
                    ...
                    <p style="margin: 0 0 12px 0; color: #475569; font-weight: 500;">{subtitle}</p>
                    <div class="time-info">
                        ⏱️ 타임코드: {seconds_to_df_timecode(start_sec)} ~ {seconds_to_df_timecode(end_sec)}<br>
                        ⏳ 재생시간: {seconds_to_min_sec(start_sec)} ~ {seconds_to_min_sec(end_sec)} ({duration}초)
                    </div>
