...
def sanitize_and_fix_highlights(
    ...
    """
    Gemini가 생성한 구간 데이터의 타임코드 무결성 및
    30~60초 조건을 엄격히 검증한다.
    """
...
def main():
    ...
    if (
        ...
        st.info(
            ...
            "GROQ_API_KEY와 GEMINI_API_KEY를 설정해 주세요."
        )
    ...
    uploaded_file = st.file_uploader(
        ...
        help=(
            "MP3, MP4, TS, MOV, M4A, WAV 형식을 지원합니다. "
    ...
    # 사용자에게 보여주는 파일명은 HTML에 직접 넣지 않음
    safe_filename = escape(
        str(uploaded_file.name)
    )

    st.success(
        f"📁 파일 선택 완료: **{safe_filename}** "
    ...
    try:
        ...
        for index, highlight in enumerate(
            ...
            start_tc = seconds_to_df_timecode(
                start_sec
            )

            end_tc = seconds_to_df_timecode(
                end_sec
            )

            start_ms = seconds_to_min_sec(
                start_sec
            )

            end_ms = seconds_to_min_sec(
                end_sec
            )
